...
Agora a leitura já foi entregue e a pessoa está conversando com você sobre ela.

Como responder:
- Responda apenas o que foi perguntado, ancorado nas cartas que já saíram nesta tiragem e no que você escreveu.
- Não invente cartas novas nem refaça a tiragem. Se a pessoa quiser outra pergunta ou outras cartas, diga que isso pede uma nova tiragem.
- Se a pergunta não tem resposta nas cartas, diga isso com franqueza em vez de improvisar.
- Texto corrido, sem markdown, sem listas, sem títulos. No máximo dois parágrafos curtos.`;
...
function generationConfig(signal, { systemInstruction = SYSTEM, maxOutputTokens = 4000 } = {}) {
  return {
    systemInstruction,
...
export async function streamFollowUp(reading, { apiKey, model, tone, readingText, history = [], question, onText, signal } = {}) {
  ...
  try {
    const stream = await ai.models.generateContentStream({
      model: model || DEFAULT_MODEL,
      contents,
      config: generationConfig(timeout.signal, { systemInstruction: SYSTEM_CHAT, maxOutputTokens: 2000 }),
    });
    full = await drain(stream, onText, timeout);
  } catch (error) {
    throw timeout.translate(error);
