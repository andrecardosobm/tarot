...
export function synthesize(draw, { question, theme, source } = {}) {
  ...
  // Em tiragem registrada à mão não houve mesa virtual nenhuma.
  const abertura = source === 'manual' ? 'Você registrou' : 'A mesa respondeu com';
  const contagem = `${total} ${total === 1 ? 'carta' : 'cartas'}`;
  parts.push(
    question
      ? `Sobre "${question}", ${abertura.toLowerCase()} ${contagem}.`
      : `${abertura} ${contagem}.`
  );
